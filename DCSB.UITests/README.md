...
| `Test-NoiseSuppression` | The rnnoise noise suppression on the microphone leg: white noise fed into the mic passes through with "Suppress background noise" off, enabling it in Settings → Sound cuts the noise floor to near zero while synthesized speech (Windows SAPI, standing in for the user's voice) still comes through, soundboard sounds bypass the suppressor entirely, and the setting persists to `config.xml` and survives an app restart. Skipped if VB-Cable is not installed or no quiet meterable output exists. |
